...
export function MondayBoardView({ boards, onBoardUpdated }: MondayBoardViewProps) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "Under Review":
      case "Demo Scheduled":
        return "bg-amber-500 hover:bg-amber-600 text-neutral-950 font-bold";
      case "Pending OCR":
      case "Contacted":
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-4 rounded-2xl bg-neutral-900/60 border border-neutral-800 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-xs font-bold text-white shadow-lg transition active:scale-95"
          >
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold transition border ${
                  isSelected
                    ? "bg-neutral-800 border-amber-500/50 text-amber-300 shadow-md"
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <input
              ...
              className="pl-8 pr-3 py-1.5 text-xs rounded-lg bg-neutral-900 border border-neutral-800 text-white placeholder-neutral-500 focus:outline-none focus:border-amber-500 transition w-48 sm:w-60"
            />
          </div>
          ...
          <select
            ...
            className="px-2.5 py-1.5 text-xs rounded-lg bg-neutral-900 border border-neutral-800 text-neutral-300 focus:outline-none transition"
          >
      ...
      <div className="rounded-2xl border border-neutral-800 bg-neutral-950/90 shadow-2xl overflow-hidden">
        {/* Table Banner Header */}
        <div className="px-5 py-3.5 bg-neutral-900/80 border-b border-neutral-800 flex items-center justify-between text-xs">
          <div className="flex items-center gap-2">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-neutral-800 bg-neutral-900/40 text-neutral-400 font-mono text-[11px] uppercase">
              ...
              {items.length === 0 ? (
                ...
                items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <td className="py-3 px-4 font-medium text-white max-w-[260px] truncate">
                        {item.name}
                      </td>
                      ...
                      <td className="py-3 px-3 text-center whitespace-nowrap">
                        {isEditing ? (
                          <div className="inline-flex items-center gap-1">
                            {["Approved", "Needs Review", "Paid"].map((st) => (
                              <button
                                ...
                                className={`px-2 py-1 rounded text-[10px] font-bold ${getStatusBadge(
                                  st
                                )}`}
                              >
                                {st}
                          ...
                          <button
                            ...
                            className={`px-3 py-1 rounded-md text-[11px] font-semibold transition shadow-sm ${getStatusBadge(
                              item.status
                            )}`}
                          >
                            {item.status}
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="rounded-2xl border border-neutral-800 bg-neutral-950 p-6 max-w-md w-full shadow-2xl space-y-4">
            ...
            <form onSubmit={handleCreateItem} className="space-y-3 text-xs">
              <div>
                <label className="block text-neutral-400 mb-1 font-medium">Item Title</label>
                <input
                  type="text"
                  placeholder="e.g. INV-2024-9982 - Data Cloud Cluster"
                  value={newItemName}
                  onChange={(e) => setNewItemName(e.target.value)}
                  className="w-full px-3 py-2 rounded-lg bg-neutral-900 border border-neutral-800 text-white placeholder-neutral-500 focus:outline-none focus:border-amber-500 transition"
                  required
                />
              </div>

              <div>
                <label className="block text-neutral-400 mb-1 font-medium">Vendor / Organization</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-neutral-400 mb-1 font-medium">Amount / Est. Value (USD)</label>
                <input
                  type="number"
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button
                  type="button"
                  onClick={() => setShowAddModal(false)}
                  className="px-3 py-1.5 rounded-lg bg-neutral-900 text-neutral-400 hover:text-white text-xs transition"
                >
                  Cancel
                </button>
                <button
